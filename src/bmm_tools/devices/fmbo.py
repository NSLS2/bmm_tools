...
def FMBO_status(motor):
    ...
    if is_FMBO_motor(motor) is False:
        ...
        text = f'\n  [white]{motor.name} is {motor.prefix}[/white]\n\n'
        for a in status_list.keys():
            ...
            if a != 'asscs':
                if signal.get() != status_list[a]:
                    string = (f'[yellow]{string: <19}[/yellow]')
            text += f'   {desc.get():26}  {string: <19}  [yellow2]{signal.get()}[/yellow2]   [bold black]{suffix}[/bold black]{padding}\n'
